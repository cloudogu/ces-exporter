...
func getKeyValues(path string, filters ...filterOption) ([]keyValue, error) {
...
func extractValues(node *client.Node, excludePrefix string) []keyValue {
	result := make([]keyValue, 0)

	if !node.Dir {
		return []keyValue{{Key: strings.TrimPrefix(node.Key, excludePrefix), Value: node.Value}}
	}
